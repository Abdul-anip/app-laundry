...
<body>
    ...
    <form action="{{ route('admin.promos.update', $promo) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="number" name="value" id="value" step="0.01" value="{{ old('value', $promo->value) }}" required>
        </div>

        <div class="form-group">
            <label for="expired_at">Tanggal Kadaluarsa (Opsional)</label>
            <input type="date" name="expired_at" id="expired_at" value="{{ old('expired_at', $promo->expired_at ? $promo->expired_at->format('Y-m-d') : '') }}">
        </div>

        <div class="form-group">
            <label>
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $promo->is_active) ? 'checked' : '' }} style="width: auto;">
                Aktifkan Promo Ini
            </label>
        </div>

        <button type="submit" class="btn">Update Promo</button>
